/* eslint
 "no-underscore-dangle": [2, { "allowAfterThis": true, "allow": [
        "_children",
        "_MPAGE_FUSION_TOOL_",
        "_MPAGE_FUSION_",
        "_emitter",
        "_propChangeHandlers",
        "_dependentPropChangeHandlers",
        "_mappedChildren",
        "_parent",
        "_downstreamEventListeners",
        "_upstreamEmitter"
    ] }],
...
/**
 * Clear the references of component.
 * it traverse each child and clears all the references from child components.
 * It clears eventListeners, propChangeHandlers, children's and parent .
 * @param {UIComponent} uiComponent - The UIComponent for which the reference need to be cleared.
 * @returns {undefined} Returns nothing.
 */
const clearComponentReferences = (uiComponent) => {
    uiComponent._emitter.removeAllListeners();
    uiComponent._parent = null;
    uiComponent._propChangeHandlers = {};
    uiComponent._dependentPropChangeHandlers = {};
    uiComponent._downstreamEventListeners = {};
    uiComponent._upstreamEmitter.removeAllListeners();
    uiComponent._mappedChildren = {};
    // clear child data
    uiComponent.getChildren().forEach((child) => {
        clearComponentReferences(child);
    });
    uiComponent._children = [];
};
...
class UIComponent {
    ...
    /**
     * Remove a mounted Fusion component from the DOM and clean up its event handlers and children's
     * @param {string} elementId - Id of the element
     * @returns {UIComponent} Current instance.
     */
    unmountComponentAtNode(elementId) {
        const node = document.getElementById(elementId);
        if (node) {
            clearComponentReferences(this);
            mRender(node, null);
        }
        return this;
    }
