...
const handleDesktopSearchBarBlur = (control, eventName) => (src, payload) => {
    ...
    // setTimeout is added so that when user clicks the clear/search icon in search bar
    // focus is removed after the click event is called.
    // else when user clicks the clear button, blur event is called first and it removes the is-focused css
    // which changes the css stacking context because of z-index and stop the click event to get called.
    setTimeout(() => {
        control.setState({ focused: false }).update();
