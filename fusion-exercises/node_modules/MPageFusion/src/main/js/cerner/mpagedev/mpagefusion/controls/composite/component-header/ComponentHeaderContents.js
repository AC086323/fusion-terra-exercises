 ...
 * @param {ComponentHeaderContents} control the current control
 * @param {label} label gets the component name
 * @returns {VDom} vdom
 */
const gapCheckIndicator = (el, control, label) =>
    el(
        "span",
        {
            ...
            role: "button",
            "aria-label": i18n().gapCheckingRequiredLabel.replace("{0}", `${label}`)
        },
...
const makeToggleItems = (items) =>
    items.map((item) => ({
        ...
        title: item.display || i18n().view.replace("{0}", `${item.icon._name}`), /* eslint no-underscore-dangle: 0 */
        label: item.display || i18n().view.replace("{0}", `${item.icon._name}`) /* eslint no-underscore-dangle: 0 */
    }));
...
class ComponentHeaderContents extends UIComponent {
    ...
    propChangeHandlers() {
        return {
            ...
            refreshButtonLabel: forwardProp(this, "label", "refreshButton"),
            contextualViewLabel: (label) => {
                this.getChild("contextualView").setProp("title", label);
                this.getChild("contextualView").setProp("label", label);
            },
            ...
            plusAddButtonTitle: (text) => {
                this.getChild("plusAddSplit").setProp("title", text);
                this.getChild("plusAddButton").setProp("title", text);
            },
            plusAddMenuLabel: (label) => {
                this.getChild("plusAddSplit").setProp("menuLabel", label);
                this.getChild("plusAddMenuOnly").setProp("title", label);
                this.getChild("plusAddMenuOnly").setProp("label", label);
            },
            plusAddButtonLabel: (label) => {
                this.getChild("plusAddSplit").setProp("label", label);
                this.getChild("plusAddButton").setProp("label", label);
            },
            refreshMenuLabel: (label) => {
                this.getChild("refreshMenu").getProp("anchorControl").setProp("title", label);
                this.getChild("refreshMenu").getProp("anchorControl").setProp("label", label);
            },
            componentMenuLabel: (label) => {
                this.getChild("componentMenu").getProp("anchorControl").setProp("title", label);
                this.getChild("componentMenu").getProp("anchorControl").setProp("label", label);
            },
            toggles: (toggles) => {
            ...
            lookbackOptions: (lookback) =>
                this.replaceMappedChild("lookbackOptions", new LookbackFilter({
                    title: i18n().lookbackRangeOptions,
                    lookbackOptions: lookback,
                ...
                [
                    el("div",
                        ...
                        [
                            ...
                            props.showGapCheckIndicator && !props.showError ? gapCheckIndicator(el, this, props.title) : [],
                            title(el, props, this),
