...
/**
 * Used to determine whether the actions should be rendered inline.
 * when size is large and and display mode is default the action will be displayed as inline
 * @param {string} size - size of the screen small/large
 * @param {string} displayMode - display mode default/compact/detail
 * @returns {boolean} True if action needs to be rendered
 * @private
 */
const shouldRenderInline = (size, displayMode) => size === SIZE.LARGE
...
const renderMain = ({ el, props, state, mapped, handleDetail }) => {
    // On all the smaller screens and when the display mode is compact and detail
    // we want to hide the "inline" actions
    // and use the "lower" actions in the disclose area.
    const renderableActions = shouldRenderInline(state.size, props.displayMode)
...
const renderDisclosure = ({ el, props, state, mapped }) => {
    // When screen is large and display mode is default, we need to hide disclosure actionable items
    // since its displayed as inline items
    const renderableActions = !shouldRenderInline(state.size, props.displayMode)
        ? renderActions(el, props, mapped)
        : [];
    return el(
...
/**
 * Check if the current size of the screen is large or not
 * @returns {boolean} True if its large
 * @private
 */
const isLargeScreen = () => mediaQuery.atLeast("MD");
...
export default class BannerItem extends UIComponent {
     ...
     */
    _handleResize() {
        const newSize = isLargeScreen() ? SIZE.LARGE : SIZE.SMALL;
